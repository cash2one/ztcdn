...
import urlparse, httplib, time, logging, base64, hmac, sha, urllib, os
...
def getPurgeReturnXmlMsg(ret):
    xmlString = ret.read().decode("utf-8")
    logger.debug("response:" + xmlString)
    xmlString = xmlString.encode('utf-8')
    doc = minidom.parseString(xmlString)
    responseNode = getChildNode(doc, 'ccsc')
    msg = getChildNodeText(responseNode, 'result')
    return msg
...
def getReturnXmlMsg(ret):
    ...
    xmlString = xmlString.encode('utf-8')
    doc = minidom.parseString(xmlString)
    responseNode = getChildNode(doc, 'ErrorResponse')
